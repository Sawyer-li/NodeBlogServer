...
exports.getAllDocTitle = function(callback) {
  const _sql = `SELECT sys_blog.id,sys_blog.title, sys_blog.create_time, sys_user.username as author
...
//获取用户下的所有文章
exports.getUserAllDocTitle = function(id, callback) {
  const _sql = `SELECT id,title,create_time FROM sys_blog WHERE account_id=${id}`;
  connect
    .querySQL(_sql)
    .then(rows => {
      callback(null, rows);
    ...
    .catch(err => {
      logger.error("getUserAllDocTitle error:");
      logger.error(err);
      callback({ type: 500 ,msg: "model getUserAllDocTitle 层错误"});
    });
};
//根据id查询文章及用户信息
exports.getDocItems = function(id, callback) {
  const _sql = `SELECT sys_blog.id,sys_blog.title, sys_blog.create_time, sys_blog.create_time, sys_blog.dochtml,  
  ...
  connect
    ...
    .catch(err => {
      ...
      logger.error(err);
      callback({ type: 500 ,msg: "model getDocItems 层错误"});
    });
};
...
exports.addDoc = function(docData, callback) {
  ...
  const _sql =
    "INSERT sys_blog(dochtml, create_time, account_id, title) VALUES(?,?,?,?)";
  ...
  connect
    ...
    .catch(err => {
      logger.error("getDocItems error:");
      logger.error(err);
      callback({ type: 500 ,msg: "model addDoc 层错误" });
    });
};
